...
@WebAppConfiguration
public class GetUserGraphsTest {
    ...
    @Test
    public void getUserGraphsTest1() throws Exception {
    	MvcResult m = mockMvc.perform(get("/getUserGraphs?name=vale&session=" + session))
                .andExpect(content().contentType(TestUtil.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$.status", is(1)))
                .andExpect(jsonPath("$.error", is("")))
                .andReturn();
    ...
    @Test
    public void getUserGraphsTest2() throws Exception {
    	MvcResult m = mockMvc.perform(get("/getUserGraphs?name=vale&session=" + session.substring(2)))
                .andExpect(content().contentType(TestUtil.APPLICATION_JSON_UTF8))
    ...
    //Test invalid rol
    @Test
    public void getUserGraphsTest3() throws Exception {
    	//change rol
    	...
    	MvcResult m = mockMvc.perform(get("/getUserGraphs?name=vale&session=" + session))
                .andExpect(content().contentType(TestUtil.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$.status", is(2)))
                .andExpect(jsonPath("$.error", is("You don'have privileges to add upload")))
                .andReturn();
    	String content = m.getResponse().getContentAsString();
    	System.out.println(content + "   " + session);
    	JSONObject jsonObject = new JSONObject(content);
